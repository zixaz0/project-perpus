...
@section('content')
    <div class="max-w-3xl mx-auto px-6 py-10">
        ...
        <div class="bg-white p-8 rounded-2xl shadow-lg border">
            <form action="{{ route('owner.pegawai.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="flex justify-end space-x-4 pt-4 border-t">
                    ...
                    <button type="submit" class="px-5 py-2 bg-indigo-600 text-white rounded-lg">Simpan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="max-w-3xl mx-auto px-6 py-10">
        ...
        <div class="bg-white p-8 rounded-2xl shadow-lg border">
            <form id="form-tambah-pegawai" action="{{ route('owner.pegawai.store') }}" method="POST"
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Password</label>
                    <input type="password" id="password" name="password" class="w-full border px-3 py-2 rounded-lg" required>
                </div>
                ...
                <div class="flex justify-end space-x-4 pt-4 border-t">
                    ...
                    <button type="button" onclick="validatePegawaiForm()"
                        class="px-5 py-2 bg-indigo-600 text-white rounded-lg">Simpan</button>
                </div>
            </form>
        </div>
    </div>
